...
def babyai_loader(batch_size):
    with open("babyai_go_to_100000_unsegmented.pkl", "rb") as f:
        data = pickle.load(f)

    dataset = BabyAIDataset(data)
...
class BabyAIDataset(Dataset):
    def __init__(self,data):
        self.data = data
        self.length = len(data["states"])

    def __len__(self):
        return self.length
    
    def __getitem__(self, index):
        states = [blosc.unpack_array() for elem in self.data["states"][index][:-1]] + [blosc.unpack_array(self.data["states"][index][-1])]
        states = np.concatenate(states, axis=0)
        return states, self.data["actions"][index]
